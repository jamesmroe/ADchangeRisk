...
tic()
#========================================================================================#
## Purpose: Simulate GAMM trajectory modelling example and PRS-AD association tests
...
DF <- readRDS(here("simulate/simulated_data.rds"))
saveres=0
...
for (iii in 1) { #1:8
  ...
  #--- load and merge with simulated genetic data ----
  slope_info <- U %>% 
    as_tibble() %>% 
    select(subject_id, rSlope, absChange) %>% 
    mutate(subject_id = as.integer(as.character(subject_id)))
  
  
  PGSS <- readRDS(here("simulate/simulated_genetic_data.rds")) %>% 
    inner_join(slope_info, by = "subject_id")
  

  PGSS = merge(PGSS, slope_info)
  ...
  if (saveres == 1) {
    print("saving")
    save("roivec","OUT","AGERELSLOPE","AGERELSLOPENOAPOE","ABSCHANGESLOPE","ABSCHANGESLOPENOAPOE",file=here(paste0("simulate/PRSADmodels_simulated.Rda")))
  }
}
# LOOPEND ROIS  ------------------------------------------------
toc()