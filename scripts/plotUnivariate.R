...
#========================================================================================#
## Purpose: plot univariate PRS-AD model results for main sample and replication sample
## Script is fully executable
## Instructions: set to analysis_sample to "main" or "replication" to plot the univariaiate results for that sample
#========================================================================================#
...
if (load_from == "excel") {
  ...
  for (sheet in roi_sheets) {
    ...
    PRSAD <- read.xlsx(source_file, sheet = sheet)
    PRSAD_NOAPOE <- read.xlsx(source_file, sheet = sheet+1)
...
for (roinum in c(1:nrois)) {
  ...
  if (plotProc == 1) {
    ...
    if (analysis_sample == "main") {
      ...
      SLOPES1 = SLOPESALL1 %>% filter(roi %in%  roinames[roinum]) #%in% roiname
      SLOPES2 = SLOPESALL2 %>% filter(roi %in%  roinames[roinum])
      SLOPES3 = SLOPESALL3 %>% filter(roi %in%  roinames[roinum])
      SLOPES4 = SLOPESALL4 %>% filter(roi %in%  roinames[roinum])
    }
    ...
    if (analysis_sample == "main") {
      ...
      (pSlopeRsqFacet = SLOPESBOTHAP %>%
         ...
         geom_col(aes(alpha=as.factor(dotalpha), y=prsq*-1,x=agerange), color=NA, position=position_dodge(width=0.5), width=0.4) +
         geom_errorbar(aes(alpha=as.factor(dotalpha), ymin=prsq_lwr*-1,ymax=prsq_upr*-1),position=position_dodge(width=0.5),size=0.3, width=0) +
         geom_point(aes(alpha=dotalphapoe, y=prsqnoAP*-1, x=agerange),col="black",position=position_dodge(width=0.5),shape=3, size=2, stroke=1) +
         geom_hline(yintercept = 0, size=0.5) +
    ...
    if (analysis_sample == "replication") {
      pSlopeFacetNoLegend = (pSlopeFacetNoLegend + theme(panel.background = element_rect(fill = "#edf4f8")))
    }
    ...
    if (analysis_sample == "replication") {
      pSlopeRsqFacetNoLegend = (pSlopeRsqFacetNoLegend + theme(panel.background = element_rect(fill = "#edf4f8")))
    }
    ...
    if (saveplots == 1) {
      ...
      if (analysis_sample == "main") {
        ...
        filename = file.path(plotdir, paste0(
          "pBetaAgeRange.facet.rsq.", roiname, ".-nTime",nTime,"_replication_text9.5.png"
        ))
      }
